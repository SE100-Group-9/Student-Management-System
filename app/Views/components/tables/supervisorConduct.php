<?php if (!empty($HanhKiem) && is_array($HanhKiem)): ?>
<div id="table-container"> 
    <table id="supervisorConduct">
        <thead>
            <tr>
                <th>Mã HK</th>
                <th>Mã học sinh</th>
        ...
        <tbody>
             <?php foreach ($HanhKiem as $index => $hanhkiem): ?>
                <tr>
                    <td><?= $hanhkiem['MaHK'] ?></td>
                    <td><?= $hanhkiem['MaHS'] ?></td>
                    <td><?= $hanhkiem['TenHS'] ?></td>
                    <td><?= $hanhkiem['Lop'] ?></td>
                    <td><?= $hanhkiem['HocKy'] ?></td>
                    <td><?= $hanhkiem['NamHoc'] ?></td>
                    <td><?= $hanhkiem['DiemHK'] ?></td>
                    <td><?= $hanhkiem['TrangThai'] ?></td>
                    


                    <td colspan="2">
                        <div class="button-container" style="display: flex; align-items: center; gap: 5px;">
                            <a href="/sms/public/supervisor/conductDetail/<?= $hanhkiem['MaHK'] ?>" title="Xem chi tiết">
                            ...
                            <a href="/sms/public/supervisor/addfault/<?= $hanhkiem['MaHK'] ?>" title="Thêm vi phạm">
...
<script>
    ...
    const tableElement = document.getElementById('supervisorConduct');
