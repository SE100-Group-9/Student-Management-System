<?php if (!empty($supervisorList) && is_array($supervisorList)): ?>
    <div id="table-container">
        <table id="directorEmployeeSupervisor">
            <thead>
                <tr>
                    <th>STT</th>
                    <th>Mã GT</th>
            ...
            <tbody>
                <?php foreach ($supervisorList as $index => $supervisor): ?>
                    <tr>
                        <td><?= $index + 1 ?></td>
                        <td><?= $supervisor['MaGT'] ?></td>
                        ...
                        <td>
                            <?= view('components/status', ['status' => $supervisor['TinhTrang']]); ?>
                        </td>

                        <td>
                            <a href="/sms/public/director/employee/supervisor/update/<?= $supervisor['MaGT'] ?>">
                            ...
                            <a href="/sms/public/director/employee/supervisor/delete/<?= $supervisor['MaGT'] ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa giám thị này?');">
...
<style>
    ...
    #directorEmployeeSupervisor a {
        text-decoration: none;
        background-color: transparent;
    }

    #directorEmployeeSupervisor svg:hover {
        cursor: pointer;
    }
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const tableElement = document.getElementById('directorEmployeeSupervisor');
